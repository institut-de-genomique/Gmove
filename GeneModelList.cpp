...
//	cout << "deleteIncludedModel " << _models.size() << endl;
	for(list<GeneModel>::iterator itOfGene = _models.begin(); itOfGene != --_models.end();++itOfGene){
		...
		for(list<GeneModel>::iterator itNext = itOfGene; itNext != _models.end() && LOOP ;++itNext){
			...
			if(itOfGene->getCDS().first == itNext->getCDS().first && itOfGene->getCDS().second == itNext->getCDS().second)//there is a first selection on same selection, if model are still in the list that means internal exons are not the same
				continue;
			...
			s32 debCDS2 = itNext->getCDS().second, finCDS2 = itNext->getCDS().first;
			if (itNext->getCDS().first < itNext->getCDS().second) {
			   debCDS2 = itNext->getCDS().first;
