...
void NetEx::bfs(s32 startId,map<s32,list < list<s32> > >& paths , s32 currentColor,set<s32>& colorNotAllowed,std::queue<s32> & Qbfs,set<s32> visited  ){
	...
	while (! Qbfs.empty()) {
		...
		if(SSRContig::VERBOSE) cout << "loop on Qbfs size "<< Qbfs.size() << endl;
		...
		for (tie(ei, ei_end) = out_edges(parentId, _graph); ei != ei_end; ++ei) {
			...
			if(SSRContig::VERBOSE)	cout <<"currentColor " << currentColor << " child Color it as "<< mapIdTranscrit.size() <<" color ";
